...
for PERCENTBLACK in "${PERCENTBLACKS[@]}"; do
	...
	# Create template
		sed -e "s|BASEDIR|$BASEDIR|g" -e "s|DATADIR|$DATADIR|g" -e "s|RADIX|$RADIX|g" -e "s|NUMROBOTS|$k|g" -e "s|R0|$R0|g" -e "s|B0|$B0|g" -e "s|PERCENT_RED|$PERCENTBLACK|g" -e "s|PERCENT_BLUE|$PERCENTWHITE|g" -e "s|DECISIONRULE|$DECISIONRULE|g" -e "s|REPETITIONS|$REPETITIONS|g" $TEMPLATE > $OUTFILE
...
for PERCENT_BLACK in "${PERCENT_BLACKS[@]}"; do
	...
	for DECISIONRULE in {1..3}; do
	    DATADIR="$DATABASE/exp_2/percentblack${PERCENT_BLACK}_decision${DECISIONRULE}_Ea${B0}/"
	    mkdir -p $DATADIR	
	    echo $DATADIR
	    for i in `seq 1 $REPETITIONS`; do
...
for k in "${NUMROBOTS[@]}"; do
	...
	for DECISIONRULE in {1..3}; do
	    ...
	    DATADIR="$DATABASE/exp_3/N${k}_percentblack${PERCENT_BLACK}_decision${DECISIONRULE}/"
	    mkdir -p $DATADIR
	    echo $DATADIR
	    for i in `seq 1 $REPETITIONS`; do
